...
class TicketGetByReporterService(BaseService):

    """
    Returns a ticket created by a reporter account.
    """
    ...
    def execute(self, ticket_id: str, account_id: str) -> dict | Exception:
        try:
            response = self.data_storage.get_reporter(ticket_id, account_id)
            ...
            self.logger.error(f'Could not get ticket `{ticket_id}` for `{account_id}`')
