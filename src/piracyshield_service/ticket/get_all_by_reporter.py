...
class TicketGetAllByReporterService(BaseService):

    """
    Returns only the tickets created by the specified reporter account.
    """
    ...
    def execute(self, account_id: str) -> bool | Exception:
        """
        Returns only the tickets created by the specified account.
        ...
        try:
            response = self.data_storage.get_all_reporter(account_id)
