...
class TicketItemFQDNGetUnprocessedByTicketService(BaseService):

    """
    Returns the unprocessed FQDN ticket items in a ticket.
    """
    ...
    def execute(self, ticket_id: str, account_id: str) -> list | Exception:
        """
        Get all the unprocessed FQDN items, of a specific ticket id, assigned to the account identifier.
        ...
        try:
            ...
            if not len(batch):
                self.logger.debug(f'No unprocessed FQDN ticket item found for account `{account_id}`')
            ...
            self.logger.error(f'Could not get all the unprocesed FQDN items for ticket `{ticket_id}` for account `{account_id}`')
