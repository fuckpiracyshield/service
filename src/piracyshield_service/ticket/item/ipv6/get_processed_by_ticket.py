...
class TicketItemIPv6GetProcessedByTicketService(BaseService):

    """
    Returns the processed IPv6 ticket items in a ticket.
    """
    ...
    def execute(self, ticket_id: str, account_id: str) -> list | Exception:
        """
        Get all the processed IPv6 items, of a specific ticket id, assigned to the account identifier.
        ...
        try:
            response = self.data_storage.get_all_ticket_items_by(
                ...
                status = TicketItemStatusModel.PROCESSED.value
            )

            batch = response.batch()

            if not len(batch):
                self.logger.debug(f'No processed IPv6 ticket item found for account `{account_id}`')
            ...
            self.logger.error(f'Could not get all the procesed IPv6 items for ticket `{ticket_id}` for account `{account_id}`')
