...
class TicketItemIPv6GetUnprocessedByTicketService(BaseService):

    """
    Returns the unprocessed IPv6 ticket items in a ticket.
    """
    ...
    def execute(self, ticket_id: str, account_id: str) -> list | Exception:
        """
        Get all the unprocessed IPv6 items, of a specific ticket id, assigned to the account identifier.
        ...
        try:
            ...
            if not len(batch):
                self.logger.debug(f'No unprocessed IPv6 ticket item found for account `{account_id}`')
