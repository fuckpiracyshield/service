...
class TicketItemIPv4GetProcessedByTicketService(BaseService):

    """
    Returns the processed IPv4 ticket items in a ticket.
    """
    ...
    def execute(self, ticket_id: str, account_id: str) -> list | Exception:
        """
        Get all the processed IPv4 items, of a specific ticket id, assigned to the account identifier.
        ...
        try:
            ...
            if not len(batch):
                self.logger.debug(f'No processed IPv4 ticket item found for account `{account_id}`')
