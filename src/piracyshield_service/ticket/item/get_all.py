...
class TicketItemGetAllService(BaseService):

    """
    Returns all the ticket items in a ticket.
    """
    ...
    def execute(self, ticket_id: str) -> list | Exception:
        try:
            response = self.data_storage.get_all(ticket_id)
            ...
            self.logger.error(f'Could not get all the ticket items for ticket `{ticket_id}`')
